...
Console support, starting with **PlayStation 5**, is in development in a private repository, following the same model MonoGame uses for its console frameworks: access is limited to registered console developers under NDA. Reach out through the issue tracker if you are a registered PlayStation developer interested in early access.
