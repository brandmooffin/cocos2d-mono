...
namespace cocos2d.particle_nodes
{
    public class CCParticleSystemCache : IDisposable, ICCUpdatable, ICCSelectorProtocol
    {
        struct AsyncStruct
        {
        ...
        void OnConfigLoad(CCParticleSystemConfig config, Action<CCParticleSystemConfig> action)
        {
            ...
            if (!string.IsNullOrEmpty(textureData))
            {
                ...
                if (dataLen != 0)
                {
                    ...
                    try
                    {
                        CCTextureCache.SharedTextureCache.AddImageAsync(imageBytes, config.TextureName, SurfaceFormat.Color, (loadedTexture) =>
                        {
