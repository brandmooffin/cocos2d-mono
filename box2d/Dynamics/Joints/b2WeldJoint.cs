    ...
    public class b2WeldJoint : b2Joint
    {
        ...
        public override void InitVelocityConstraints(b2SolverData data)
        {
            ...
            if (data.step.warmStarting)
            {
                ...
                vA -= mA * P;
                wA -= iA * (b2Math.b2Cross(ref m_rA, ref P) + m_impulse.z);

                vB += mB * P;
                wB += iB * (b2Math.b2Cross(ref m_rB, ref P) + m_impulse.z);
            }
            else
            {
                m_impulse.SetZero();
        ...
        public override bool SolvePositionConstraints(b2SolverData data)
        {
            ...
            else
            {
                ...
                cA -= mA * P;
                aA -= iA * (b2Math.b2Cross(ref rA, ref P) + impulse.z);

                cB += mB * P;
                aB += iB * (b2Math.b2Cross(ref rB, ref P) + impulse.z);
            }
